...
import FedexIcon from '../../assets/FedexIcon';
...
const FedexTrackingResource = Resource('fedex-tracking');

class FedexTracking extends Component {
  fetchTrackingInfo = () => {
    ...
    FedexTrackingResource.read({ trackingNo: toolState.trackingNo })
      .then(data => {
        let newToolState = {
          ...toolState,
          loading: false,
      ...
      .catch(error => {
        console.log(error);
        const errorMessage = 'Could not access Fedex data.';
  ...
  render() {
    ...
    let externalLink = attributes.NMG.fedexTrackingUrl;
    let homeLink = attributes.NMG.fedexHomeUrl;
    if (trackingNo) {
      externalLink = `${externalLink}?tracknumbers=${trackingNo}`;
    } else {
    ...
    return (
      <ToolContent taskSid={taskSid}>
        <ToolHeader
          label={
            <Grid container>
              <FedexIcon variant="blue" className={classes.headerFedexIcon} />
        ...
        {trackingInfo && (
          <>
            ...
            <ToolScrollable scrollKey={`${taskSid}-fedex`}>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(styles)(FedexTracking));
